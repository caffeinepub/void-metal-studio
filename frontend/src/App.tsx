...
import { useGetCallerUserProfile, useIsBanned, useBanUser } from './hooks/useQueries';
...
import Canvas, { type CanvasFile, type GeneratedResult } from './components/Canvas';
...
export default function App() {
  ...
  const [genModalOpen, setGenModalOpen] = useState<'music' | 'video' | 'ignite' | null>(null);
  ...
  const handleViolation = useCallback(() => {
    ...
    banUserMutate(undefined, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['isBanned'] });
      },
    });
  }, [isAuthenticated, banUserMutate, queryClient]);
  ...
  const handleExport = () => {
    ...
    if (latestGen) {
      exportContent({
        ...
        mimeType: latestGen.type === 'music' || latestGen.type === 'ignite' ? 'audio/mpeg' : 'video/mp4',
      });
    } else if (firstFile) {
      exportContent({
        blob: firstFile.file,
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          />
          <p className="gothic-label text-sm animate-ember-flicker">AWAKENING THE VOID...</p>
        </div>
      </div>
    );
  }
  ...
  if (showProfileSetup) {
    return (
      <div
        ...
        style={{ background: 'oklch(0.06 0.005 20)' }}
      >
        <ProfileSetup onComplete={() => queryClient.invalidateQueries({ queryKey: ['currentUserProfile'] })} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: 'url(/assets/generated/gargoyle-dragon-face.dim_1920x1080.png)',
      ...
      <div
        className="fixed inset-0 pointer-events-none z-0"
        style={{
          background: 'radial-gradient(ellipse at 50% 0%, transparent 30%, oklch(0.04 0.002 20 / 0.7) 100%)',
        }}
      />
      ...
      <header
        ...
        style={{ borderBottom: '1px solid oklch(0.3 0.08 25)', borderLeft: 'none', borderRight: 'none', borderTop: 'none' }}
      >
        {/* Logo */}
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h1 className="gothic-title text-lg leading-none">VOID METAL STUDIO</h1>
            <p
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2">
            <img
              src="/assets/generated/emoji-steel.dim_128x128.png"
              alt=""
              className="w-5 h-5"
              style={{ filter: 'drop-shadow(0 0 4px oklch(0.55 0.25 25 / 0.5))' }}
            />
            <span
              className="font-cinzel text-xs"
      ...
      <main className="relative z-10 flex-1 flex flex-col gap-3 p-3 md:p-4">
        ...
        <div className="relative flex justify-center">
          <button
            ...
            style={
              activeEffect
                ? { borderColor: 'oklch(0.55 0.22 25)', color: 'oklch(0.75 0.25 25)' }
                : {}
            }
          >
            <img
              ...
              alt=""
              className="w-5 h-5"
              style={{ filter: 'drop-shadow(0 0 6px oklch(0.65 0.28 25 / 0.7))' }}
            />
            EFFECTS
            {activeEffect && (
              <span
      ...
      <footer
        ...
        style={{ borderTop: '1px solid oklch(0.25 0.06 25)', borderLeft: 'none', borderRight: 'none', borderBottom: 'none' }}
      >
        <div className="flex items-center gap-3">
          {/* Emoji picker button */}
          <div className="relative">
            ...
            <EmojiPicker
              ...
              onSelect={(symbol) => {
                // Copy to clipboard for use in any input
      ...
      <input
        ...
        multiple
        className="hidden"
        onChange={(e) => { processFileInput(e.target.files); e.target.value = ''; }}
      />
      <input
        ref={videoInputRef}
        type="file"
