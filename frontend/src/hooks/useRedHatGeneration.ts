...
const SHEPHERD_ENDPOINT = 'https://shepherd-main.apps.cluster.example.com/api/generate';
...
export function useRedHatGeneration() {
  ...
  const generate = async (payload: GenerationPayload): Promise<GenerationResult | null> => {
    ...
    try {
      ...
      if (response && response.ok) {
        ...
        if (contentType.includes('audio')) {
          ...
          genResult = { type: payload.type, blob, url: URL.createObjectURL(blob) };
        } else if (contentType.includes('video')) {
          const blob = await response.blob();
          genResult = { type: payload.type, blob, url: URL.createObjectURL(blob) };
        } else {
          const data = await response.json();
          genResult = { type: payload.type, url: data.url, message: data.message };
        }
      } else {
